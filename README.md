...
| `just dev`        | Development mode with frontend hot reload: start Docker dependencies, run backend with `go run ./cmd/server`, and run Vite (`pnpm --dir web run dev`). API calls are proxied from Vite to `http://localhost:8080`.                                                                                                                |
