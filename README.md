...
| Command           | Purpose                                                                                                                                                                                                                                                                                                                                |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `just build`      | Build `bin/audit-log` and the embedded web assets.                                                                                                                                                                                                                                                                                     |
| `just run`        | Production-like local run: build binary + embedded SPA, start Docker dependencies, run `./bin/audit-log`.                                                                                                                                                                                                                             |
| `just dev`        | Development mode with frontend hot reload: start Docker dependencies, run backend with `go run ./cmd/server`, and run Vite (`npm --prefix web run dev`). API calls are proxied from Vite to `http://localhost:8080`.                                                                                                                |
...
| `just functional-api` | Godog suite. Build, `docker compose up --wait`, start `bin/audit-log` in the background, wait until the gRPC port accepts connections, run Godog scenarios against that address, then stop the app and `docker compose down`. Preserves `go test` exit code. Sets `AUDIT_LOG_FUNCTIONAL_GRPC_ADDR` (default `127.0.0.1` + `AUDIT_LOG_SERVER_PORT`). Extra args go to `go test`. |
| `just functional-web` | Not applicable: audit-log has no browser suite. Stub kept so tooling shared with control-plane can call it. |
| `just functional` | Every functional suite: `functional-api` + `functional-web`. |
| `just lint`       | `golangci-lint run ./...` (config in `.golangci.yml`, version pinned in `.mise.toml`). Depends on `build` because `internal/web` embeds the SPA. |
